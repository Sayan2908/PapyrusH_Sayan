...
# Store the path of the folder where extracted json files are present
folder_path = "./Extracted_json_files"

# define invoice.csv file
filename = "./invoice.csv"    # Here I have given path according to me you can give the path where you want to save your file
...
for element in os.listdir(folder_path):
    ...
    # Now define some variables, which we are going to extract from the list
    ...
    # Iterate through every element in new_list[]
    for text in new_list:
        ...
        if '@'in text:
            ...
            # if customer_email[-1] == "o":
            #     customer_email = customer_email + "m"
            # if customer_email[-1] == "c":
            #     customer_email = customer_email + "om"
            # if customer_email[-1] == customer_email[-2]:
            #     customer_email = customer_email[:-1]

        # Searching for 'XXX-XXX-XXXX' pattern in the string for phone_number
    ...
    # We have deleted unwanted phrases so customer_name should be at index 0
    ...
    # Using a loop we will extract the transaction item, quantity, rate

    last_index = (len(new_list))-1
    trans_starting_index = last_index - 3*number_of_transaction # Multiplying with 3 as there are 3 elements item,quantity and rate
